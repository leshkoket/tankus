...
//====== Geom =====
	...
	Geom.distanceOf=function( firstX, firstY, secondX, secondY ){
		var x = parseFloat( firstX-secondX );
		var y = parseFloat( firstY-secondY );
	...
	Geom.angleRadOf=function( sourceX, sourceY, targetX,targetY ){
		var x = parseFloat( targetX-sourceX );
		var y = parseFloat( targetY-sourceY );
	...
	Geom.correctAngleRad=function( rad ){
		rad = Geom.rem( rad, Math.PI*2 );
	...
	Geom.optimalRotatingDirectionRad=function( sourceRad, targetRad ) {
		...
		if( ((Math.PI*2 - maxRad) + minRad) < (maxRad - minRad) ){
			return (sourceRad < targetRad) ? -1 : +1;
		}
