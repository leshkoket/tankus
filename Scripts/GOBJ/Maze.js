...
class Maze extends GameObject {
	...
	cellPointByIndexes( indexes ){
		...
		return new Point( size.x() * Number(indexes.x()), size.y() * Number(indexes.y()) );
	}
...
class MazeDrawer extends GameObjectComponent {
	...
	onUpdate(){
		...
		*/
		...
		let startX = Math.ceil( this.game().camera().worldX( 0 ) / cellSize.x()) - EXTEND;
		let startY = Math.ceil(this.game().camera().worldY( 0 ) / cellSize.y()) - EXTEND;
		let endX = startX + (gfxNode.width / cellSize.x() + 2*EXTEND);
		let endY = startY + (gfxNode.height / cellSize.y() + 2*EXTEND);
		
		
		gfx.setTransform();
		let image = this.game().resources().loadResource("Image", "Images/Brick.png");
		if( !image ) return;
		for( let y = startY; y <= endY; y++ ){
			for( let x = startX;  x <= endX; x ++ ){
				if( !this.linkedObject().cellExists(x,y) ) continue;
				//if( !this._canDrawCellAtIndex(gfxNode, x,y,cellSize.x(),cellSize.y()) ) continue;
				this._drawCellAtIndex( gfx, image, x,y, cellSize.x(), cellSize.y() );
			}
		}
		gfx.setTransform();
	}
...
//#private:
	...
	_drawCellAtIndex( gfx, image, x,y, drawW,drawH ){
		let drawX = this.game().camera().displayX( drawW*x );
		let drawY = this.game().camera().displayY( drawH*y );
		...
		if( cell <= 0 ){
			...
			this._drawTheObstacleCell( gfx, image, cell, drawX,drawY,drawW,drawH );
		}
	}
	
	
	_drawTheFreeCell( gfx, image, cell, drawX,drawY, drawW,drawH ){
		const t=3;
	...
	_drawTheObstacleCell( gfx, image, cell, drawX,drawY,drawW,drawH ){
		const t = 4;
